...
pub fn generate_random_charges(
    ...
    println!(
        "Generated {} charges with power-law distribution:",
        charges.len()
    );
    println!(
        "  Largest: {:.2}, Smallest: {:.2}, Ratio: {:.1}x",
        charge_values[0],
        charge_values[charge_values.len() - 1],
...
pub fn simulate_equilibrium(
    ...
    for iteration in 0..config.max_iterations {
        ...
        for i in 0..charges.len() {
            ...
            let force_magnitude =
                (total_forces[i].0.powi(2) + total_forces[i].1.powi(2) + total_forces[i].2.powi(2))
                    .sqrt();
        ...
        if max_force < config.convergence_threshold {
            println!(
                "Converged after {} iterations (max force: {:.2e})",
                iteration + 1,
                max_force
            );
            break;
        ...
        for i in 0..charges.len() {
            ...
            velocities[i].0 =
                velocities[i].0 * config.damping_factor + total_forces[i].0 * config.time_step;
            velocities[i].1 =
                velocities[i].1 * config.damping_factor + total_forces[i].1 * config.time_step;
            velocities[i].2 =
                velocities[i].2 * config.damping_factor + total_forces[i].2 * config.time_step;
