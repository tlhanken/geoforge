...
use crate::tectonics::boundary_analysis::{
...
use crate::tectonics::island_removal::{IslandRemovalConfig, IslandRemovalStats, IslandRemover};
...
use crate::tectonics::{GenerationMethod, TectonicPlateGenerator};
use bincode;
...
impl TectonicMetadata {
    ...
    pub fn read_from_buffer(
        buffer: &[u8],
        cursor: &mut usize,
    ) -> Result<TectonicMetadata, Box<dyn std::error::Error>> {
...
impl WorldMap {
    ...
    pub fn import_png(&mut self, png_path: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        println!(
            "🖼️ Importing tectonic plates with motion from PNG: {}",
            png_path
        );
        ...
        for y in 0..self.height {
            for x in 0..self.width {
                ...
                let (plate_id, _direction, _speed) =
                    if let Some(&existing) = color_to_plate.get(&color) {
                        existing
                    } else {
        ...
        println!(
            "📊 Detected {} unique plates with motion vectors",
            num_plates
        );
        ...
        for (plate_id, pixels) in plate_pixels {
            ...
            let projection =
                crate::map::terrain::MapProjection::global_equirectangular(self.width, self.height);
            let (lat, lon) = projection.pixel_to_coords(centroid_x, centroid_y);

            // Get motion from color mapping
            let (motion_direction, motion_speed) =
                plate_motion.get(&plate_id).copied().unwrap_or((0.0, 2.5)); // Default fallback
    ...
    pub fn generate_tectonics(
        ...
        let mut generator =
            TectonicPlateGenerator::with_seed(self.width, self.height, num_plates, self.seed)
                .map_err(|e| -> Box<dyn std::error::Error> { Box::new(e) })?;

        // Generate the plates
        generator
            .generate(GenerationMethod::Electrostatic)
            .map_err(|e| -> Box<dyn std::error::Error> { Box::new(e) })?;
    ...
    ) -> Result<IslandRemovalStats, Box<dyn std::error::Error>> {
        // Ensure tectonics have been generated
        if self.tectonics.is_none() {
    ...
    pub fn generate_geology(
        ...
        let config = config.unwrap_or_default();
        let generator =
            crate::geology::GeologyGenerator::new(config, self.seed, self.planetary_params.clone());
    ...
    fn calculate_plate_stats_from_map(
        &self,
        tectonic_map: &TerrainMap<u16>,
        plate_seeds: &[PlateSeed],
    ) -> HashMap<u16, PlateStats> {
        let mut plate_areas = HashMap::new();

        for y in 0..self.height {
            for x in 0..self.width {
                let (lat, _lon) = tectonic_map.projection.pixel_to_coords(x, y);
                let pixel_area = tectonic_map
                    .projection
                    .pixel_area_km2_with_radius(lat, self.planetary_params.radius_km);
    ...
    pub fn load_from_file(filepath: &str) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        if flags & 0x01 != 0 {
            ...
            for item in tectonic_data.iter_mut().take(width * height) {
                *item = u16::from_le_bytes([buffer[cursor], buffer[cursor + 1]]);
                cursor += 2;
            }
        ...
        if flags & 0x02 != 0 {
            ...
            for item in elevation_data.iter_mut().take(width * height) {
        ...
        if flags & 0x01 != 0 && cursor < buffer.len() {
            worldmap.tectonic_metadata =
                Some(TectonicMetadata::read_from_buffer(&buffer, &mut cursor)?);
        }
    ...
    fn test_png_export_import_roundtrip() {
        ...
        for (orig_id, orig_stat) in original_stats {
            ...
            for (imp_id, imp_stat) in imported_stats {
                if orig_stat.area_km2 == imp_stat.area_km2
                    && !plate_mapping.values().any(|&v| v == *imp_id)
                {
        ...
        assert!(
            ...
            "Plate boundaries were not exactly preserved through PNG roundtrip"
        );
